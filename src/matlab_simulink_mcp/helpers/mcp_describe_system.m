...
% Use cell arrays of structs so jsonencode ALWAYS produces JSON arrays
% (even for a single element/connection), and to avoid struct-array
...
for j = 1:n
    ...
    portStruct = struct('name', char(nm), 'index', idx);
    if ~isempty(portType)
        portStruct.type = char(portType);
    end
    portsOut{j, 1} = portStruct;
end
end
